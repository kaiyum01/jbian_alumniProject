...
if($login_session_usertype==1){  $user_type_cond=" "; }else{  $user_type_cond=" and user_type=$login_session_usertype and user_name='".$login_session_username."'"; }
...
if(	isset($_POST['action']) &&
	...
	$batchName2		=$_POST['cbo_batch_name1'];
	$status2		=$_POST['status1'];

	if($action_save=="save_data")
	{
	  	$query = mysql_query("insert into admin_user_create(user_name,pass_word,user_email,user_type,batch_name,status_active,insert_date,inserted_by) values ('$name2','$password2','$email2','$usertype2','$batchName2','$status2','$insert_and_update_date','$login_session_user_id')");
	  	if($query==1){
	  	echo $msg_save;
...
if(isset($_GET['action'])){
	...
	{
		$i=0;
		$result=mysql_query("select * from admin_user_create where is_deleted=0  $user_type_cond order by id DESC");
		?>
...
if(	isset($_POST['action']) &&
	...
	{
	//update query 
	  $query_update = mysql_query("update admin_user_create SET user_name='$name2', pass_word='$password2', user_email='$email2', user_type='$usertype2', batch_name='$batchName2', status_active='$status2',update_date='$insert_and_update_date',updated_by='$login_session_user_id' where id='$update_id2'");
	  if($query_update==1){
	  	echo $msg_update;
...
if(isset($_GET['action']) && isset($_GET['search_value1'])){
	...
	{
		$i=0;
		$result=mysql_query("select * from admin_user_create where user_name like '%$search_value2%' and is_deleted=0  $user_type_cond order by id DESC");
		?>
		<table class="table table-hover scroll">
    <thead>
      <tr>
