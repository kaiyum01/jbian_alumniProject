...
if(	isset($_POST['action']) &&
	isset($_POST['cbo_video_cat_name1']) && 
 	isset($_POST['link_title1']) && 
 	isset($_POST['link_url1'])
 	//isset($_POST['contact1'])
 	){
	$action_save		=$_POST['action'];
	$cbo_video_cat_name2		=$_POST['cbo_video_cat_name1'];
	$link_title2		=mysql_escape_string($_POST['link_title1']);
	$link_url2			=mysql_escape_string($_POST['link_url1']);
	
	if($action_save=="save_data")
	{
	  	$query = mysql_query("insert into video_create(video_title,video_url,video_category,insert_date,inserted_by) values ('$link_title2','$link_url2','$cbo_video_cat_name2','$insert_and_update_date','$login_session_user_id')");
...
if(isset($_GET['action'])){
	...
	{
		$i=0;
		$result=mysql_query("select * from video_create where is_deleted=0 and status_active=1 order by id DESC");
		?>
...
if(isset($_GET['action'])){
	 ...
	 if($action_data_populate_to_form =="getdata")
	 {
		...
		$idd=$_GET['idd'];
		$sql= mysql_query("select * from video_create where id=$idd");
...
if(	isset($_POST['action']) &&
	...
	{	
	//update query 
	  $query_update = mysql_query("update video_create SET video_title='$link_title2', video_url='$link_url2',video_category='$cbo_video_cat_name2',update_date='$insert_and_update_date',updated_by='$login_session_user_id' where id='$update_id2'");
...
if(isset($_POST['action'])){
	 ...
	 if($action_delete=="delete_data_action")
	 {
		...
		$sql_delete= mysql_query("update video_create SET status_active=0, is_deleted=1,update_date='$insert_and_update_date',updated_by='$login_session_user_id' where id='$delete_id'");
		if($sql_delete==1)
		{
			echo $msg_delete;
		}
...
if(isset($_GET['action']) && isset($_GET['search_value1'])){
	...
	{
		$i=0;
		$result=mysql_query("select * from video_create where video_title like '%$search_value2%' and is_deleted=0 and status_active=1 order by id DESC");
		?>
		<table class="table table-hover scroll">
    <thead>
     <tr>
