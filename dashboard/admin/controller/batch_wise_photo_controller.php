	...
	)
{
	...
	$batchname2		=$_POST['cbo_batch_name'];
	...
	{ // update 1st condition
		if ($_FILES["file"]["name"]=="") {
			...
			$sql_update= mysql_query("update batch_create_photo_dtls SET batch_name='$batchname2', update_date='$insert_and_update_date',updated_by='$login_session_user_id' where id='$update_id'");
			if($sql_update==1)
			{
...

	{
		...
		{
			$update_pic_path='uploadBatchPhotos/'.$update_pic;
			unlink($update_pic_path);
			if (file_exists("uploadBatchPhotos/" . $_FILES["file"]["name"])) {
			echo $_FILES["file"]["name"] . " <span id='invalid'><b>already exists.</b></span> ";
			}
			else
			{
			$sourcePath = $_FILES['file']['tmp_name']; // Storing source path of the file in a variable
			$targetPath = "uploadBatchPhotos/".$_FILES['file']['name']; // Target path where file is to be stored
			$sql_update= mysql_query("update batch_create_photo_dtls SET batch_name='$batchname2',batch_photo_link='$targetPath', update_date='$insert_and_update_date',updated_by='$login_session_user_id' where id='$update_id'");
			//$query = mysql_query("insert into batch_create_mst(batch_name, 	batch_banner_pic,batch_reward,batch_title,insert_date,inserted_by) values ('$batchname2','$targetPath','$batchreward2','$titlename2','$insert_and_update_date','$login_session_user_id')");
	...
	//return batch 
	$return_batch=fnc_pickup_data_from_db_table("batch_name,batch_name","batch_create_photo_dtls","is_deleted=0"); 
	$return_batch_arr=array();
	foreach($return_batch as $aa)
		{
			$return_batch_arr[$aa[0]] =$aa[1]; //$aa=['id'].
		}
	/*	if($return_batch_arr[$batchname2]==$batchname2)
		{
	...
	*/
			...
			{
				$query = mysql_query("insert into batch_create_photo_dtls(batch_name,insert_date,inserted_by) values ('$batchname2','$insert_and_update_date','$login_session_user_id')");
			...
			else // if without picture or cover picture
			{
...

				{
					...
					else
					{
						...
						$sourcePath = $_FILES['file']['tmp_name']; // Storing source path of the file in a variable
						$targetPath = "uploadBatchPhotos/".$_FILES['file']['name']; // Target path where file is to be stored
						$query = mysql_query("insert into batch_create_photo_dtls(batch_name,batch_photo_link,insert_date,inserted_by) values ('$batchname2','$targetPath','$insert_and_update_date','$login_session_user_id')");
...
if(isset($_GET['action'])){
	...
	if($action_view_data=="list_view")
	{
		...
		$i=0;
		$result=mysql_query("select * from batch_create_photo_dtls where is_deleted=0 and status_active=1 $batchNameCond order by id DESC");
		?>
	...
	?>
      <tr style="cursor: pointer;">
        ...
        if($data[2]=="")
        {
        	echo 'uploadBatchPhotos/no-image.jpg';
        }
        else
        {
        	 echo  $data[2];
...
if(isset($_GET['action'])){
	 ...
	 if($action_data_populate_to_form =="getdata")
	 {
		...
		$idd=$_GET['idd'];
		$sql= mysql_query("select * from batch_create_photo_dtls where id=$idd");
...
if(isset($_POST['action'])){
	 ...
	 if($action_delete=="delete_data_action")
	 {
		...
		$sql_delete= mysql_query("update batch_create_photo_dtls SET status_active=0, is_deleted=1,update_date='$insert_and_update_date',updated_by='$login_session_user_id' where id='$delete_id'");
		if($sql_delete==1)
		{
			echo $msg_delete;
			unlink($imagePath);
...
if(isset($_GET['action']) && isset($_GET['search_value1'])){
	...
	if($action_search=="search_list_view")
	{
		if ($login_session_usertype!=1) { $batchNameCond= "and batch_name=$login_session_batch";} else{ $batchNameCond="";}
		$i=0;
		$result=mysql_query("select * from batch_create_photo_dtls where batch_name like '%$search_value2%' and is_deleted=0 and status_active=1 $batchNameCond order by id DESC");
		?>
		<table class="table table-hover scroll">
    <thead>
     <tr>
