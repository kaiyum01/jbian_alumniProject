...
if(	isset($_POST['action']) &&
 	...
 	isset($_POST['email1']) &&
 	isset($_POST['mobile_11']) && 
 	isset($_POST['mobile_21']) &&
 	isset($_POST['fb_link_url1']) 

 	//isset($_POST['contact1'])
 	){
	$action_save	=$_POST['action'];
	$address2		=mysql_escape_string($_POST['address1']);
	$email2			=mysql_escape_string($_POST['email1']);
	$mobile_12		=mysql_escape_string($_POST['mobile_11']);
	$mobile_22		=mysql_escape_string($_POST['mobile_21']);
	$fb_link_url2	=mysql_escape_string($_POST['fb_link_url1']);
	
	if($action_save=="save_data")
	{
	  	$query = mysql_query("insert into contact_create(address,fb_link,mobile_1,mobile_2,email,insert_date,inserted_by) values ('$address2','$fb_link_url2','$mobile_12','$mobile_22','$email2','$insert_and_update_date','$login_session_user_id')");
...
if(isset($_GET['action'])){
	...
	{
		$i=0;
		$result=mysql_query("select * from contact_create where is_deleted=0 and status_active=1 order by id DESC");
		?>
		...
		<thead>
		  <tr>
			...
			<th style="width: 150px; text-align: center;">fb link</th>
	...
	?>
      <tr style="cursor: pointer;">
        ...
        <td style="width: 150px; text-align: center;"><?php echo $data[2]; ?></td>
        <td style="width: 150px; text-align: center;"><?php echo $data[3]; ?></td>
        <td style="width: 150px; text-align: center;"><?php echo $data[4]; ?></td>
...
if(isset($_GET['action'])){
	 ...
	 if($action_data_populate_to_form =="getdata")
	 {
		...
		$idd=$_GET['idd'];
		$sql= mysql_query("select * from contact_create where id=$idd");
...
if(	isset($_POST['action']) &&
	...
	{	
	//update query 
	  $query_update = mysql_query("update contact_create SET address='$address2', fb_link='$fb_link_url2', mobile_1='$mobile_12', mobile_2='$mobile_22', email='$email2',update_date='$insert_and_update_date',updated_by='$login_session_user_id' where id='$update_id2'");
	  mysql_query("SET character_set_results = 'utf8', character_set_client = 'utf8', character_set_connection = 'utf8', character_set_database = 'utf8', character_set_server = 'utf8'");
...
if(isset($_POST['action'])){
	 ...
	 if($action_delete=="delete_data_action")
	 {
		...
		$sql_delete= mysql_query("update contact_create SET status_active=0, is_deleted=1,update_date='$insert_and_update_date',updated_by='$login_session_user_id' where id='$delete_id'");
		if($sql_delete==1)
		{
			echo $msg_delete;
		}
...
if(isset($_GET['action']) && isset($_GET['search_value1'])){
	...
	{
		$i=0;
		$result=mysql_query("select * from contact_create where address like '%$search_value2%' and is_deleted=0 and status_active=1 order by id DESC");
		?>
		<table class="table table-hover scroll">
    <thead>
     <tr>
