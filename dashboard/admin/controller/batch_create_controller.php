...
if(isset($_FILES["file"]["type"]) && 
	isset($_POST['cbo_batch_name']) && 
 	isset($_POST['txt_batch_reward']) &&
 	isset($_POST['txt_batch_desc']) &&
 	isset($_POST['txt_title']) &&
	...
	)
{
	...
	$batchreward2	=mysql_escape_string($_POST['txt_batch_reward']);
	$batchdesc2		=mysql_escape_string($_POST['txt_batch_desc']);
	$titlename2		=mysql_escape_string($_POST['txt_title']);
	...
	{ // update 1st condition
		if ($_FILES["file"]["name"]=="") {
			...
			$sql_update= mysql_query("update batch_create_mst SET batch_name='$batchname2',batch_reward='$batchreward2',batch_desc='$batchdesc2',batch_title='$titlename2', update_date='$insert_and_update_date',updated_by='$login_session_user_id' where id='$update_id'");
...

	{
		...
		{
			$update_pic_path='uploadBatchCover/'.$update_pic;
			unlink($update_pic_path);
			if (file_exists("uploadBatchCover/" . $_FILES["file"]["name"])) {
			echo $_FILES["file"]["name"] . " <span id='invalid'><b>already exists.</b></span> ";
			}
			else
			{
			$sourcePath = $_FILES['file']['tmp_name']; // Storing source path of the file in a variable
			$targetPath = "uploadBatchCover/".$_FILES['file']['name']; // Target path where file is to be stored
			$sql_update= mysql_query("update batch_create_mst SET batch_name='$batchname2',batch_reward='$batchreward2',batch_desc='$batchdesc2',batch_banner_pic='$targetPath',batch_title='$titlename2', update_date='$insert_and_update_date',updated_by='$login_session_user_id' where id='$update_id'");
			mysql_query("SET character_set_results = 'utf8', character_set_client = 'utf8', character_set_connection = 'utf8', character_set_database = 'utf8', character_set_server = 'utf8'");
	...
	//return batch 
	$return_batch=fnc_pickup_data_from_db_table("batch_name,batch_name","batch_create_mst","is_deleted=0"); 
	...
	foreach($return_batch as $aa)
		...
		if($return_batch_arr[$batchname2]==$batchname2)
		{
			...
			{
				$query = mysql_query("insert into batch_create_mst(batch_name,batch_reward,batch_desc,batch_title,insert_date,inserted_by) values ('$batchname2','$batchreward2','$batchdesc2','$titlename2','$insert_and_update_date','$login_session_user_id')");
				mysql_query("SET character_set_results = 'utf8', character_set_client = 'utf8', character_set_connection = 'utf8', character_set_database = 'utf8', character_set_server = 'utf8'");
			...
			else // if without picture or cover picture
			{
...

				{
					...
					else
					{
						...
						$sourcePath = $_FILES['file']['tmp_name']; // Storing source path of the file in a variable
						$targetPath = "uploadBatchCover/".$_FILES['file']['name']; // Target path where file is to be stored
						$query = mysql_query("insert into batch_create_mst(batch_name, 	batch_banner_pic,batch_reward,batch_desc,batch_title,insert_date,inserted_by) values ('$batchname2','$targetPath','$batchreward2','$batchdesc2','$titlename2','$insert_and_update_date','$login_session_user_id')");
						mysql_query("SET character_set_results = 'utf8', character_set_client = 'utf8', character_set_connection = 'utf8', character_set_database = 'utf8', character_set_server = 'utf8'");
...
if(isset($_GET['action'])){
	...
	?>
      <tr style="cursor: pointer;">
        ...
        if($data[2]=="")
        {
        	echo 'uploadBatchCover/no-image.jpg';
        }
        else
        {
        	 echo  $data[2];
