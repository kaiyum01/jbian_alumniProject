...
if(	isset($_POST['action']) &&
 	...
 	isset($_POST['lib_cat_name1']) && 
 	isset($_POST['book_description1'])
	...
	$lib_cat_name2		=$_POST['lib_cat_name1'];
	$book_description2	=mysql_escape_string($_POST['book_description1']);
	
	if($action_save=="save_data")
	{
	  	$query = mysql_query("insert into online_lib_create(book_link_url, book_link_title,book_category,book_description,insert_date,inserted_by) values ('$link_url2','$link_title2','$lib_cat_name2','$book_description2','$insert_and_update_date','$login_session_user_id')");
...
if(isset($_GET['action'])){
	...
	{
		$i=0;
		$result=mysql_query("select * from online_lib_create where is_deleted=0 and status_active=1 order by id DESC");
		?>
		...
		<thead>
		  <tr>
			...
			<th style="width: 500px; text-align: center;">book name</th>	
			<th>Action</th>
		  </tr>
		</thead>
    <tbody>
	...
	?>
      <tr style="cursor: pointer;">
      	...
      	<td style="width: 150px;"><?php echo $library_category_arr[$data[3]]; ?></td>
        <td style="width: 500px; text-align: center;"><a href="<?php echo $data[2]; ?>" rel="develop" target="_blank"><?php echo $data[1]; ?></a></td>
...
if(isset($_GET['action'])){
	 ...
	 if($action_data_populate_to_form =="getdata")
	 {
		...
		$idd=$_GET['idd'];
		$sql= mysql_query("select * from online_lib_create where id=$idd");
...
if(	isset($_POST['action']) &&
	...
	{	
	//update query 
	  $query_update = mysql_query("update online_lib_create SET book_link_title='$link_title2', book_link_url='$link_url2', book_category='$lib_cat_name2', book_description='$book_description2',update_date='$insert_and_update_date',updated_by='$login_session_user_id' where id='$update_id2'");
	  mysql_query("SET character_set_results = 'utf8', character_set_client = 'utf8', character_set_connection = 'utf8', character_set_database = 'utf8', character_set_server = 'utf8'");
...
if(isset($_POST['action'])){
	 ...
	 if($action_delete=="delete_data_action")
	 {
		...
		$sql_delete= mysql_query("update online_lib_create SET status_active=0, is_deleted=1,update_date='$insert_and_update_date',updated_by='$login_session_user_id' where id='$delete_id'");
		if($sql_delete==1)
		{
			echo $msg_delete;
		}
...
if(isset($_GET['action']) && isset($_GET['search_value1'])){
	...
	{
		$i=0;
		$result=mysql_query("select * from online_lib_create where book_link_title like '%$search_value2%' and is_deleted=0 and status_active=1 order by id DESC");
		?>
		<table class="table table-hover scroll">
    <thead>
     <tr>
