...
<head>
  ...
  Copyright: 	www.abdulkaiyum.com  
	-->
  <title>jbEX | Videos Create</title>
  <meta charset="utf-8">
...
function data_send(){
  ...
  var cbo_video_cat_name    = $("#cbo_video_cat_name").val();
  var link_title            = $("#txt_link_title").val();
  var link_url              = $("#txt_link_url").val();
  // Returns successful data submission message when the entered information is stored in database.
  var dataString = '&link_title1='+ link_title + '&link_url1='+ link_url + '&cbo_video_cat_name1='+ cbo_video_cat_name;
  //alert(dataString);
   if(cbo_video_cat_name==0){ $("#cbo_video_cat_name_red").css("color","#EE5269" );}else{ $("#cbo_video_cat_name_red").css("color","green" );}
  ...
  if(link_title==''|| link_url=='' || cbo_video_cat_name==0){
  ...
  else
  {
    ...
    $.ajax({
      type: "POST",
      url: "controller/video_create_controller.php",
      ...
      { 
        ...
        $("#cbo_video_cat_name").val(0);    
        $("#txt_link_title").val('');
        $("#txt_link_url").val('https://www.youtube.com/embed/');
        //default color of start mark
        $("#txt_link_url_red").css("color","#555555" );
        $("#txt_link_title_red").css("color","#555555" );
        $("#cbo_video_cat_name_red").css("color","#555555" );
	  ...
	  $.ajax({    //create an ajax request to load_page.php
			...
			success: function(results){											
				// alert (results['batch_reward']);
				 $('#txt_link_url').val(results['video_url']);
         $('#txt_link_title').val(results['video_title']);
         $('#cbo_video_cat_name').val(results['video_category']);		
...
function data_update(){
  ...
  var link_url     = $("#txt_link_url").val();
	...
	// Returns successful data submission message when the entered information is stored in database.
	var dataString = '&link_title1='+ link_title + '&link_url1='+ link_url + '&cbo_video_cat_name1='+ cbo_video_cat_name + '&update_id1='+ id;
	//alert(dataString);
   if(cbo_video_cat_name==0){ $("#cbo_video_cat_name_red").css("color","#EE5269" );}else{ $("#cbo_video_cat_name_red").css("color","green" );}
  if(link_title==''){ $("#txt_link_title_red").css("color","#EE5269" );}else{ $("#txt_link_title_red").css("color","green" );}
  if(link_url==''){$("#txt_link_url_red").css("color","#EE5269" ); }else{$("#txt_link_url_red").css("color","green" ); }
...
<div class="container-fluid page_path_div" style=" background-color:#fff; height:auto; width:98%;color:#666666;">
  ...
  <div class="pagename" style="float:right;">Page: Video create</div><hr/>
    ...
    <div id="mySidenav" class="sidenav">
      ...
      <li><a class="menu active"  href="video_create.php">Video</a></li>
...
-->
  ...
  <div class="col-md-6">
      <h2>Video create / update form</h2>
      <p>you can create / update video by using this form</p>
  <form class="form-horizontal">
   <div class="form-group">
      <label class="control-label col-sm-4" for="video_category">Video Category:</label>
      <div class="col-sm-8">
      <div class="input-group">
        <select style="margin-left:8px;" class="form-control" id="cbo_video_cat_name" name="cbo_video_cat_name">
      <?php
      foreach ($video_category_arr as $video_category_arr_key => $video_category_arr_name) {
      ?><option value="<?php echo $video_category_arr_key; ?>" <?php /*?><?php if( $imvalue==$updata['Timportancy'])  echo "selected"; ?> <?php */?>><?php echo $video_category_arr_name ?>
      </option> 
      <?php
      }
      ?>
        </select> 
       <span class="input-group-addon"><span class="glyphicon glyphicon-asterisk" id="cbo_video_cat_name_red"></span></span>
        </div>
      </div>
    </div>
    <div class="form-group">
      <label class="control-label col-sm-4" for="linktitle">Video Title:</label>
      <div class="col-sm-8">
       <div class="input-group">
        <input type="text" class="form-control" id="txt_link_title" name="txt_link_title" placeholder="Enter video title name">
